...
function process_cf7_and_send_sms( $contact_data ){
	...
	if (isset($_POST[$phone_field])) {
        ...
        if ($submission) {
            // get submission data
            $nexmo_key = get_option('cspd_cf7_nexmo_key');
            $nexmo_secret = get_option('cspd_cf7_nexmo_secret');
            $nexmo_from = get_option('cspd_cf7_nexmo_from');
           ...
           if (get_option('cf7_enable_user_sms')=="checked") {
                    ...
                    $user_sms_resp = cspd_cf7_send_nexmo_sms($nexmo_key,$nexmo_secret,$user_phone_num,$nexmo_from,$user_sms_txt);
                    // echo "<br/><pre>"; print_r($user_sms_resp); echo "</pre><br/>";
           ...
           if (get_option('cf7_enable_admin_sms')=="checked") {
                   ...
                   // echo $admin_sms_txt;
                    ...
                    $admin_sms_resp = cspd_cf7_send_nexmo_sms($nexmo_key,$nexmo_secret,$admin_phone_num,$nexmo_from,$admin_sms_txt);
                    // echo "<br/><pre>"; print_r($admin_sms_resp); echo "</pre>";
           }
